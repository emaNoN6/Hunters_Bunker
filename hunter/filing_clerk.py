...
def file_new_lead(log_queue, standardized_report):
    """
    Performs the full, multi-step ritual for filing a new lead.
    ...
    log_queue.put(f"[FILING_CLERK]: Receiving new lead for filing: '{title}'")
    ...
    if not lead_uuid:
        log_queue.put(f"[FILING_CLERK ERROR]: Failed to create router entry for '{title}'. Aborting file.")
        return False
    ...
    if not success:
        log_queue.put(f"[FILING_CLERK ERROR]: Failed to file staging data for lead '{lead_uuid}'.")
        # In a future version, we might want to roll back the router entry here.
        return False

    log_queue.put(f"[FILING_CLERK]: Successfully filed new lead '{title}' with UUID: {lead_uuid}")
    return True
